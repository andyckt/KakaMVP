...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[API] Starting v0 generation for prompt:", prompt);
    ...
    (async () => {
      try {
        // Use the generate-with-v0.ts script
        const scriptPath = path.join(process.cwd(), "scripts", "generate-with-v0.ts");
        ...
        child.stdout.on("data", async (data) => {
          ...
          for (const line of lines) {
            ...
            // Parse v0 messages
            if (line.includes('__V0_MESSAGE__')) {
              const jsonStart = line.indexOf('__V0_MESSAGE__') + '__V0_MESSAGE__'.length;
              try {
                const message = JSON.parse(line.substring(jsonStart).trim());
                await writer.write(
                  encoder.encode(`data: ${JSON.stringify({ 
                    type: "claude_message", // Use same type for UI compatibility
            ...
            // Regular progress messages
              ...
              if (output && 
                ...
                // Extract chat ID
                const chatMatch = output.match(/Chat created with ID: ([a-z0-9_]+)/);
                if (chatMatch) {
                  chatId = chatMatch[1];
                }
              }
        ...
        // Send completion with preview URL and chat ID
        if (previewUrl) {
          await writer.write(
            encoder.encode(`data: ${JSON.stringify({ 
              type: "complete", 
          ...
          console.log(`[API] Generation complete. Preview URL: ${previewUrl}, Chat ID: ${chatId}`);
        } else {
